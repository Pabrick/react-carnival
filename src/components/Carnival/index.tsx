...
import { Arrow, Bar, CarnivalWrapper, Marker, Zone } from './styles';
...
const Carnival = ({ onSuccess, onFailure, size, duration, random = false }: CarnivalProps) => {
	...
	return (
		<CarnivalWrapper data-testid='carnival' onMouseDownCapture={onMouseDown} onMouseUpCapture={onMouseUp}>
			<Bar ref={barRef}></Bar>
			<Zone data-testid='zone' $width={`${zone.width}px`} $left={`${zone.left}px`}></Zone>
			<Marker data-testid='marker' ref={markerRef} style={{ left: `${posX}%` }}>
				<Arrow $height={`${barHeight / 2}px`} />
			</Marker>
		</CarnivalWrapper>
	);
};
